...
const Helpers = {
  ...
  userAgentFindOne(arr) {
      ...
      var ua = process.server ? headers['user-agent'] : navigator.userAgent
  ...
  isIE() {
    let headers = useRequestHeaders()
    var ua = process.server ? headers['user-agent'] : window.navigator.userAgent;
  ...
  isMac() {
    let headers = useRequestHeaders()
    var ua = process.server ? headers['user-agent'] : navigator.platform;
  ...
  checkForTablet() {
      let headers = useRequestHeaders()
      var sw = process.server ? headers['user-agent'] : screen.width,
        sh = process.server ? headers['user-agent'] : screen.height,
        dev_width = process.server ? headers['user-agent'] : Math.min(sw, sh);
  ...
  checkForTabletMedia() {
    let headers = useRequestHeaders()
    var wv = process.server ? headers['user-agent'] : window.innerWidth;
    if (750 < wv && wv <= 1050) {
      return true;
    }
    return false;
